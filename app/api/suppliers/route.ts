...
export async function GET(request: NextRequest) {
  ...
  try {
    const suppliers = await readData<Supplier>("suppliers")
    const businessSuppliers = suppliers.filter((supplier) => supplier.businessId === businessId)
    return NextResponse.json(businessSuppliers)
  } catch (error) {
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    await addRecord("suppliers", newSupplier)
